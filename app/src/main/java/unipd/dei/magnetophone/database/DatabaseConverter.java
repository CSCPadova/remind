...
public class DatabaseConverter {
     ...
     * in file formato xml
     */
    public static void exportDatabase(Context cont) {
        //interrogo il database
        DatabaseHelper dbHelper = new DatabaseHelper(cont);
        SQLiteDatabase db = dbHelper.getReadableDatabase();

        //prendo tutte le info sulle canzoni
        Cursor songs = db.query(DatabaseHelper.SONG, null, null, null, null, null, null, null);
        songs.moveToFirst();
        ...
        try {
            ...
            for (int i = 0; i < songs.getCount(); i++) {
                ...
                if (songs.getString(16) != null)
                    ...
                    serializer.attribute(null, PDF, "");

                //prendo l'id della canzone
                int songId = songs.getInt(0);

                //interrogo il database per conoscere le track legate alla canzone
                String selection = DatabaseHelper.TRACKSONG + " = " + songId;
                Cursor tracks = db.query(DatabaseHelper.TRACK, null, selection, null, null, null, null, null);
                tracks.moveToFirst();
                ...
                selection = DatabaseHelper.VIDEOSONG + " = " + songId;
                Cursor video = db.query(DatabaseHelper.VIDEO, null, selection, null, null, null, null, null);
                if (video.getCount() > 0) {
                    video.moveToFirst();
                ...
                selection = DatabaseHelper.PHOTOSSONG + " = " + songId;
                Cursor photos = db.query(DatabaseHelper.PHOTOS, null, selection, null, null, null, null, null);
