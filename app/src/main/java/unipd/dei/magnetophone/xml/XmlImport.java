...
public class XmlImport {
     ...
     * @param dbHelper
     */
    public void importazioneXML(Context cont, DatabaseHelper dbHelper, boolean force) {
        ...
        try {
            ...
            //conflitti
            cursor = db.query(DatabaseHelper.XML, null, null, null, DatabaseHelper.NOMEFILE, null, null, null);//prendo tutti gli XML nel database
                ...
                for (int j = 0; j < songsDirectories.length; j++) {
                    ...
                    if (xmlFileList != null)//se è != null, ossia se c'è almeno 1 file xml
                    {
                        ...
                        for (int i = 0; i < fileListLength; i++) {
                            ...
                            String[] select = {DatabaseHelper.XMLID, DatabaseHelper.NOMEFILE, DatabaseHelper.DATAMODIFICA};
                            String where = DatabaseHelper.NOMEFILE + "=\"" + nomeFile + "\"";

                            query = db.query(DatabaseHelper.XML, select, where, null, null, null, null, null);
                            ...
                            if (numberOfCollision != 0)//se c'è già un file con lo stesso nome nel database
                            {
                                ...
                                if (lastModifiedStorage > lastModifiedDb | force)//se nel database è più vecchio, elimino e sostituisco
                                {
